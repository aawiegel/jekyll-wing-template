...
As data sets for this recommendation system, I used the [Stanford Dogs image data set](http://vision.stanford.edu/aditya86/ImageNetDogs/) along with some web scraped images and breed descriptions from a few different websites. To help avoid this situation in the future, I developed a dog breed recommender using two models: an image classifier and a topic model. The image classifier was trained via transfer learning with the convolutional neural network Xception and weights from ImageNet. For each of the 114 dog breeds included in the image data set, I took the average of the last layer for the images in the test set to create the appearance representations. The topic model was developed from text scraped from several different dog websites and Wikipedia. I dubbed the process of converting both the image and text data into vector reprsentations "dog2vec". 

<!-- ## Recommendations -->

<!-- After creation of the dog vectors for each breed, a user-provided image and text description could be -->
